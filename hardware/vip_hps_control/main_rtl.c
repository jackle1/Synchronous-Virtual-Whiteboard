...
//Camera
#define CAM2HPS_BASE (0X60)
#define CAM2HPS_STATUS CAM2HPS_BASE+3

volatile int * PIX_RDY_PTR;
volatile int * CAM_PTR;
...
int main()
{
    ...
    // initCamera();
    // DISABLE_CAMERA();
    printf("Checking camera turned off");
...
//         }
        ...
        if (GET_SW_VAL() & SW_8)
        {
            ...
            while (!(getKeyAH(KEY_BASE_PTR) & KEY3_MASK))
            {
                while (getKeyAH(KEY_BASE_PTR) & KEY3_MASK);
...
void initBridge(void)
{
    ...
    *LEDR_BASE_PTR = 0;
    ...
    CAM_PTR = (unsigned int *) (lw_bridge_ptr + CAM2HPS_BASE);
    PIX_RDY_PTR = (unsigned int *) (lw_bridge_ptr + CAM2HPS_STATUS);
...
int paintVGAPictureRTL(const char *file, volatile uint32_t *vga_base)
{
    FILE *fd = NULL;
    fd = fopen(file, "rb");
    if (fd == NULL)
...
void saveCurrentPicture(const char *file, volatile uint32_t *vga_base)
{
    ...
    uint32_t * pixels = (uint32_t*) malloc(sizeof(uint32_t) * (640*480));

    CAM_PTR = (unsigned int *) (lw_bridge_ptr + CAM2HPS_BASE);
    PIX_RDY_PTR = (unsigned int *) (lw_bridge_ptr + CAM2HPS_STATUS);

    // unsigned short pix_16;
    int k = 0;

    for(int i = 0; i < 480; i++) {
        for(int j = 0; j < 640; j++) {
            *CAM_PTR = (j << 10 | i & 1023);
            pixels[k] = *PIX_RDY_PTR;
            fprintf(fd, "%u ", pixels[k]);
            k++;

            // pix_16 = (unsigned short) pixels[k]
        }
    }
