...
uint8_t OV8865_read_cmos_sensor_8(uint16_t Addr){
	const uint8_t device_address = MIPI_I2C_ADDR;
	uint8_t Value;

	//OC_I2CL_Write(I2C_OPENCORES_CAMERA_BASE, device_address, SWAP16(Addr), (uint8_t *)&Value, sizeof(Value));
	OC_I2CL_Read(I2C_OPENCORES_CAMERA_BASE, device_address, Addr, (uint8_t *)&Value, sizeof(Value));

	return (Value);
}


void OV8865_write_cmos_sensor_8(uint16_t Addr, uint8_t Value){
	const uint8_t device_address = MIPI_I2C_ADDR;
	//OC_I2CL_Write(I2C_OPENCORES_CAMERA_BASE, device_address, SWAP16(Addr), (uint8_t *)&Value, sizeof(Value));
	OC_I2CL_Write(I2C_OPENCORES_CAMERA_BASE, device_address, Addr, (uint8_t *)&Value, sizeof(Value));
}


void OV8865_write_AF(uint8_t msb, uint8_t lsb){
	// VCM149C
	const uint8_t device_address = MIPI_AF_I2C_ADDR;
	OC_I2C_Write(I2C_OPENCORES_CAMERA_BASE, device_address, msb, (uint8_t *)&lsb, sizeof(lsb));
}
...
//    	printf("%x:  ",ii);
//    	uint8_t data = 0x30;
//        OC_I2C_Write(I2C_OPENCORES_CAMERA_BASE, ii, 0x30,  (uint8_t *)&data, 1);
//        sleep_us(10000);
//    }
//
     ...
     for(i=0;i<num;i++){
    	 ...
    	 if (MipiCameraReg[i].Type == TIME_DELAY)   sleep_us(MipiCameraReg[i].Data*100);
